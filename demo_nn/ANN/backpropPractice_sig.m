...
change.wts_ho = nan(maxEpochs,1);
change.wts_ih = nan(maxEpochs,1);
record.wts_ho = nan(maxEpochs,1);
record.wts_ih = nan(maxEpochs,1);
...
for iter = 1:maxEpochs    
    ...
    for j = 1:m
        ...
        %% back prop algorihtm 
        % forward prop
        hval = sigmoid(wts_ih * x');
        pred = wts_ho * hval;
        
        % compute delta
        delta_ho = pred - y;
        delta_ih = wts_ho' * delta_ho .* (1-hval) .* hval;
        
        % adjust the weights
        wts_ho = wts_ho - (delta_ho * hval)' * lr; 
