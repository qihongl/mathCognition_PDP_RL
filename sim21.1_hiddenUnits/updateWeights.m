...
if p.experienceReply
    ...
    if a.bufferUsage > a.usage_startReplay
        ...
        for i = 1 : p.replay_batchSize
            ...
            delta_o = TD_Err;
            delta_h = delta_o .* a.wts_ho(buffer(memoryIdx).a_cur,:)' .* (1-(hact.^2));

            % adjust the weights 
            a.wts_ho(buffer(memoryIdx).a_cur,:) = a.wts_ho(buffer(memoryIdx).a_cur,:) - p.lrate * (delta_o * hact)';
            a.wts_ih = a.wts_ih - p.lrate * delta_h * buffer(memoryIdx).s_cur; 
    ...
    [a.dfRwd, hact] = computeFutureReward(w.vS.visInput, a.curRwd, w.done);

    % compute delta
    TD_Err = a.act(a.choice) - a.dfRwd;
    delta_o = TD_Err;
    delta_h = delta_o * a.wts_ho(a.choice,:)' .* (1-(hact.^2));

    % adjust the weights 
    a.wts_ho(a.choice,:) = a.wts_ho(a.choice,:) - p.lrate * (delta_o * hact)';
    a.wts_ih = a.wts_ih - p.lrate * delta_h * w.vS.oldInput;     
end
